...
const GameList = ({ handleLogout }) => {
  ...
  const fetchGames = async () => {
    ...
    try {
      const response = await fetch("https://gamelist-api.vercel.app/games", {
        headers: {
  ...
  const handleSave = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://gamelist-api.vercel.app/games/${selectedGame.id_game}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteGame = async (gameId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://gamelist-api.vercel.app/games/${gameId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
